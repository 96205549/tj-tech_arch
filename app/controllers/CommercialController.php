...
class CommercialController extends Controller
{
    ...
    {
        if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
    ...
    public function newFolderAction()
    {
        ...
        //die(print_r($this->session->get("commercial")));
        $key = $this->request->get('key');
        //die(print_r($key));
        $iduser = $this->session->get('userid');
        $pseudo = $this->session->get('pseudo');
        $idserv = $this->session->get('commercial');
            ...
            if (empty($foldername)) {
                ...
                if ($folder) {
                    ...
                    if ($folder->save()) {
                        ...
                        return $this->response->redirect("commercial/updateFolder?id=" . base64_encode($folder->iddos));
                    } else {
                        $this->flashSession->error("Echec de modification du dossier");
                        return $this->response->redirect("commercial/updateFolder?id=" . $folder->iddos);
                    }
                }
            }
            $this->response->redirect("commercial/openFolder?id=" . base64_encode($idfolder));
        }
        $folder = dossier::findFirst(["iddos =?0 ", "bind" => ["0" => $idfolder]]);
        if ($folder == TRUE) {
            $this->view->folder = $folder;
        } else {
            return $this->response->redirect("commercial/newFolder");
        }
    }
    /*
    ...
    public function openFolderAction()
    {
        ...
        if (!(int) $id) {
            ...
            if ($doc) {
                ...
                $this->response->redirect("commercial/newFolder");
            }
        }
        $this->view->page = "commercial";
    }
    ...
    public function childFolderAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->response->redirect("commercial/openfolder?id=" . base64_encode($parent));
        }
    }
    ...
    public function childFileAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        if ($this->request->isPost()) {
            ...
            $this->response->redirect("commercial/openfolder?id=" . base64_encode($folderid));
        }
    }
    /*
     * update folder name
    ...
    public function deleteFolderAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        $this->response->redirect("commercial/openFolder?id=" . base64_encode($idp));
        $this->view->page = "commercial";
    }
    /*
    ...
    public function deleteFolderMAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        $this->response->redirect("commercial/mesFolders");
        $this->view->page = "commercial";
    }
    /*
    ...
    public function deleteFoldersOAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        $this->response->redirect("commercial/open?id=" . base64_encode($idp));
        $this->view->page = "commercial";
    }
    ...
    public function deleteFileAction()
    {
        ...
        ///$id = explode("_", $val[0]);
        if (!(int) $id) {
            return $this->response->redirect('commercial/mesFolders');
        }
        ...
        $this->response->redirect("commercial/openFolder?id=" . base64_encode($idf));
        $this->view->page = "commercial";
    }
    /*
    ...
    public function updateFileAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($docFile->update()) {
                ...
                return $this->response->redirect("commercial/updateFile?id=" . base64_encode($idfile) . '.' . base64_encode($docFile->iddos));
            } else {
    ...
    public function childFoldersAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        if ($this->request->isPost()) {
            ...
            $this->response->redirect("commercial/open?id=" . base64_encode($parent));
        }
    }
    ...
    public function childFilesAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->response->redirect("commercial/open?id=" . base64_encode($folderid));
        }
    }
    /*
     * update folder name
    ...
    public function updateFoldersChildAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->response->redirect("commercial/open?id=" . $parent);
        }
    }
    /*
     * modification des fichier d'un dossier
    ...
    public function updateFilesAction()
    {
       ...
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        if ($this->request->isPost()) {
            ...
            if ($docFile->update()) {
                $this->flashSession->success("Le fichier a &eacute;t&eacute; mise &agrave; jour avec succ&egrave;s");
                return $this->response->redirect("commercial/updateFiles?id=" . base64_encode($idfile) . '.' . base64_encode($docFile->iddos));
            } else {
                $this->flashSession->error("Echec de la mise &agrave; jour du fichier");
                return $this->response->redirect("commercial/updateFiles?id=" . base64_encode($idfile) . '.' . base64_encode($docFile->iddos));
            }
        ...
        if ($this->request->isPost()) {
            ...
            if (empty($foldername)) {
                ...
                if ($folder) {
                    ...
                    if ($folder->save()) {
                        $this->flashSession->success("Le dossier &agrave; &eacute;t&eacute; Modifier avec success");
                        return $this->response->redirect("commercial/updateFolders?id=" . base64_encode($folder->iddos));
                    } else {
                        $this->flashSession->error("Echec de modification du dossier");
                        return $this->response->redirect("commercial/updateFolders?id=" . base64_encode($folder->iddos));
                    }
            ...
            $this->response->redirect("commercial/open?id=" . base64_encode($idfolder));
        }
        $folder = dossier::findFirst(["iddos =?0 ", "bind" => ["0" => $idfolder]]);
        if ($folder == TRUE) {
            $this->view->folder = $folder;
    ...
    public function deleteFilesOAction()
    {
       if (!$this->session->get('userid') || $this->session->get("commercial") == "0") {
        ...
        $this->response->redirect("commercial/open?id=" . base64_encode($idf));
        $this->view->page = "commercial";
    }
    /*
        ...
        if ($this->request->isPost()) {
            ...
            if ($taille == 0) {
                if ($type == "doc") {
                    ...
                    return $this->response->redirect("commercial/sharedoc?id=" . $id);
                } else {
                    $this->flashSession->error("Veuillez choisir l&apos;utilisateur auquel vous souhaitez envoyer le courier");
                    return $this->response->redirect("commercial/sharefile?id=" . base64_encode($id));
                }
            }
            $i = 0;
            ...
            if ($i == $taille) {
                if ($type == "doc") {
                    ...
                    return $this->response->redirect("commercial/sharedoc?id=" . base64_encode($id));
                } else {
